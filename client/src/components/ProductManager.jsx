  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        className={`product-modal-panel max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl ${isClosing ? 'is-closing' : ''}`}
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-medium text-emerald-600">Ürün Yönetimi</p>
            <h2 className="mt-1 text-xl font-semibold">{product ? 'Ürünü düzenle' : 'Ürün ekle'}</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <div className="flex justify-end gap-3 border-t border-slate-100 pt-5 sm:col-span-2"><button type="button" onClick={() => handleClose()} className="rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-100 transition-colors">Vazgeç</button><button type="submit" disabled={saving || uploading || processing} className="rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-700 disabled:opacity-50 transition-colors">{processing ? 'Optimize ediliyor...' : uploading ? 'Yükleniyor...' : saving ? 'Kaydediliyor...' : product ? 'Değişiklikleri kaydet' : 'Ürün oluştur'}</button></div>
        </form>
      </div>
    </div>
  );
