const ProductCard = ({ product, onSelect }) => (
  <button
    type="button"
    className={`public-product ${product.isFeatured ? 'is-featured' : ''} ${!product.isAvailable ? 'is-unavailable' : ''}`}
    onClick={() => onSelect(product)}
  >
    <span className="public-product__visual" aria-hidden="true">{product.image ? <img src={product.image} alt="" /> : '✦'}</span>
    <span className="public-product__body">
      ...
      {product.shortDescription && <span className="public-product__description">{product.shortDescription}</span>}
