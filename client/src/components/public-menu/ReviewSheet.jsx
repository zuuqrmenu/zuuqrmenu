...
const ReviewSheet = ({ username, onClose, onSubmitted }) => {
  ...
  const submit = async (event) => {
    ...
    setSaving(true);
    try { await publicMenuService.submitReview(username, form); setSuccess(true); onSubmitted?.(); } catch (err) { setError(err.response?.data?.error || 'Değerlendirme gönderilemedi.'); } finally { setSaving(false); }
  };
